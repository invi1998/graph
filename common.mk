# .PHONY:all clean

# ifeq：表示条件语句的开始，并指定一个条件表达式。表达式包含两个参数，以逗号分隔，表达式以圆括号括起，表示判断表达式与某个值是否相等
ifeq ($(DEBUG),true)
# -g 是生成调试信息。GNU调试器可以利用该信息
CC = g++ -std=c++20 -g
VERSION = release
endif

# $(wildcard *.c) 表示扫描当前目录下所有的.c文件
# SRCS = GraphTheory.c
SRCS = $(wildcard *.cpp)

# OBJS = GraphTheory.o 这样一个一个增加.o文件太麻烦，下行换一种写法，把字符串中.c换成.o
# 解释：OBJS = $(SRCS:.cpp=.o) 变量替换函数，把所有的*.cpp文件替换为*.o文件。该函数和 OBJS = $(patsubst %.cpp,%.o,$(SRCS))具有相同效果。
OBJS = $(SRCS:.cpp = .o)

# 把字符串中的.c替换为.d
# DEPS = GraphTheory.d
DEPS = $(SRCS:.cpp = .d)

# 可以指定BIN文件的位置，addprefix是增加前缀函数
# BIN = D:/st/GraphTheory
BIN := $(addprefix $(BUILD_ROOT)/,$(BIN))

# 定义存放obj文件的目录，目录统一放在一个位置才方便后续链接。不让整到各个子目录里不好链接
# 注意下面这个字符串，末尾不要有空格等，否则会报语法错误
LINK_OBJ_DIR = $(BUILD_ROOT)/app/link_obj
DEP_DIR = $(BUILD_ROOT)/app/dep

# -p是递归创建目录，没有就创建，有就不需要创建
$(shell mkdir -p $(LINK_OBJ_DIR))
$(shell mkdir -p $(DEP_DIR))

# 然后我们需要把目标文件生成到上述目标文件目录中去，利用addprefix增加这个前缀
# 处理后形如/GraphTheory/app/link_obj/graph.o
# := 再解析阶段直接复制常量字符串【立即展开】，而 = 是再运行阶段，实际使用变量时再进行求值【延时展开】
OBJS := $(addprefix $(LINK_OBJ_DIR)/,$(OBJS))
DEPS := $(addprefix $(DEP_DIR)/,$(DEPS))

# 找到目录中的所有的.o文件（编译出来的.o文件）
LINK_OBJ = $(wildcard $(LINK_OBJ_DIR)/*.o)
# 因为构建以来关系时，这个app目录下这个.o文件还没有编译出来，所以LINK_OBJ是缺少这个.o文件的，所以哟啊把这个.o文件加进来
LINK_OBJ += $(OBJS)

# ----------------------------------------------------------------------------------------------------------------------------------------

# make找到第一个目标开始执行【每个目标就是我们要生成的文件，其实都是定义一种依赖关系，目标的格式为：】
# 目标：目标依赖【可以省略】
# 		要执行的命令【可以省略】
# 如下这行将会是开始执行的入口，执行就找到依赖项$(BIN)去执行了，同时这里也依赖了$(DEPS), 这样就会生成很多.d文件了
all:$(DEPS) $(OBJS) $(BIN)

# 这里诸多的.d文件被包含进来，每个.d文件里面都记录着一个.o文件所依赖哪些.c文件和.h文件
# 内容诸如：ngx.o: ngx.c ngx.h
# 做这一步的目的就是为了实现，即便.h文件被修改了，也要让make重新编译我们的工程，否则修改了.h文件，make却不会重新编译，这是不行的
# 有必要先判断这些文件是否存在，不然make会报.d文件找不到
ifneq ("$(wildcard $(DEPS))", "")		# 如果不为空，$(wildcard)是函数【获取匹配模式文件名】，这里 用于比较是否为 ""
include $(DEPS)
endif


# ------------------------------------------------------------------1begin------------------------------------------------------------------
# $(BIN):$(OBJS)
$(BIN):$(LINK_OBJ)
	@echo "--------------------------build $(VERSION) mode------------------------------!!!"

# 一些变量：$@:目标， $^: 所有目标依赖
# gcc -o 是生成可执行文件
	$(CC) -o $@ $^ -lpthread


# ------------------------------------------------------------------1end------------------------------------------------------------------


# ------------------------------------------------------------------2begin------------------------------------------------------------------
# %.o:%.c
$(LINK_OBJ_DIR)/%.o:%.cpp
# gcc -c是生成.o目标文件，-I可以指定头文件的路径
# 如下不排除有其他字符串,所以从其中专门把.c过滤出来
# $(CC) -o $@ -c $^
	$(CC) -I$(INCLUDE_PATH) -o $@ -c $(filter %.cpp,$^)

# ------------------------------------------------------------------2end------------------------------------------------------------------

# ------------------------------------------------------------------3begin------------------------------------------------------------------
# 我们现在希望当修改一个.h时，也能够让make自动重新编译我们的项目，所以，我们需要指明让.o依赖于.h文件
# 那一个.o依赖于哪些.h文件，我们可以用“gcc -MM c程序文件名” 来获得这些依赖信息并重定向保存到.d文件中
# .d文件中的内容可能形如：nginx.o: nginx.c ngx_func.h
# %.d:%.c
$(DEP_DIR)/%d:%.cpp
# gcc -MM $^ > $@
#.d文件中的内容形如：nginx.o: nginx.c ngx_func.h ../signal/ngx_signal.h，但现在的问题是我们的.o文件已经放到了专门的目录
# 所以我们要正确指明.o文件路径这样，对应的.h,.c修改后，make时才能发现，这里需要用到sed文本处理工具和一些正则表达式语法，不必深究
#gcc -MM $^ | sed 's,\(.*\)\.o[ :]*,$(LINK_OBJ_DIR)/\1.o:,g' > $@
#echo 中 -n表示后续追加不换行
	echo -n $(LINK_OBJ_DIR)/ > $@
#	gcc -MM $^ | sed 's/^/$(LINK_OBJ_DIR)&/g' > $@
#  >>表示追加
#	gcc -I$(INCLUDE_PATH) -MM $^ >> $@
	$(CC) -I$(INCLUDE_PATH) -MM $^ >> $@

#上行处理后，.d文件中内容应该就如：/mnt/hgfs/linux/nginx/app/link_obj/nginx.o: nginx.c ngx_func.h ../signal/ngx_signal.h


# ------------------------------------------------------------------3end------------------------------------------------------------------

# ------------------------------------------------------------------nbegin------------------------------------------------------------------
#clean:			
#rm 的-f参数是不提示强制删除
#可能gcc会产生.gch这个优化编译速度文件
#	rm -f $(BIN) $(OBJS) $(DEPS) *.gch
# ------------------------------------------------------------------nend------------------------------------------------------------------
